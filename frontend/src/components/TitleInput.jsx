import React, { useState, forwardRef } from "react";

export const TitleInput = forwardRef(({ title, setTitle, ...props }, ref) => {
  return (
    <div className="w-full">
      <label htmlFor="title" className="mb-1 inline-block">
        Title
        <sup>*</sup>
      </label>
      <input
        ref={ref} // Forwarding ref to the input element
        id="title"
        type="text"
        className="w-full border bg-transparent px-2 py-2 outline-none border-[#14d0ff]"
        {...props}
      />
    </div>
  );
});

export const DescriptionInput = forwardRef(
  ({ description, setDescription, ...props }, ref) => {
    return (
      <div className="w-full">
        <label htmlFor="desc" className="mb-1 inline-block">
          Description
          <sup>*</sup>
        </label>
        <textarea
          ref={ref} // Forwarding ref to the textarea element
          id="desc"
          className="h-40 w-full resize-none border border-[#14d0ff] bg-transparent px-2 py-1 outline-none"
          {...props}
        ></textarea>
      </div>
    );
  }
);
