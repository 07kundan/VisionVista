...
const ChannelAbout = () => {
  ...
  const channelDetails = [
    ...
    {
      icon: FaGlobe,
      text: `https://${channelAbout?.username}`,
      link: `https://${channelAbout?.username}`,
    },
  ...
  return (
    <div className=" text-white p-6 rounded-lg shadow-lg max-w-md">
      {" "}
      <h2 className="text-xl font-semibold mb-4 text-cyan-300">About</h2>{" "}
      ...
      <div className="space-y-3">
        ...
        {channelDetails.map((detail, index) => (
          <div key={index} className="flex items-center">
            {" "}
            <detail.icon className="text-cyan-400 mr-3 text-xl" />{" "}
            {detail.link ? (
              <Link
                to={detail.link}
                className="text-cyan-200 hover:text-cyan-400 transition duration-300"
              >
                {" "}
                {detail.text}{" "}
              </Link>
